...
function validatePass(regex: RegExp) {
    return function (target:any, propertyKey: string, descriptor: PropertyDescriptor){
        const originalSet = descriptor.set;
...
class User {
    ...
    // @validateName(1) // match
    @validateName(3) // mismatch user4
    set name(val: string) { this._name = val; }
    // @validateAge(1, 150) // match
    // @validateAge(1, 100) // mismatch user1
    set age(val: number) { this._age = val; }
    // @validatePass(/^[a-zA-Z0-9]+$/g) // match
    // @validatePass(/^[a-zA-Z0-9!@]+$/g) // mismatch user2 and user3
