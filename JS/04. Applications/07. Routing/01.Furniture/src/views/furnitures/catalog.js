...
export default async function Catalog(){
    try {
        const response = await fetch("http://localhost:3030/data/catalog").then(response => response.json());    
        if (response.code >= 400){
            throw new Error(response.message);
        }
        render(CatalogTemplate(response), main);
    }
    catch(err){ alert(err); }
}

function CatalogTemplate(source){
    return html`
        <div class="container">
